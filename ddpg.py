...
class OrnsteinUhlenbeckActionNoise:
    def __init__(self, mu, sigma=0.5, theta=.15, dt=1e-2, x0=None):
        self.theta = theta
        self.mu = mu
        self.sigma = sigma
        self.dt = dt
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        for i in range(20):
            ...
            while not d:
                env.render()
                obs, r, d, _ = env.step(actor.predict(np.reshape(s, (1, actor.s_dim)))[0])
            env.close()


if __name__ == '__main__':
    id = gym_hypercube.dynamic_register(n_dimensions=2,
                                        env_description={'high_reward_value': 10,
    ...
    parser.add_argument('--max-episode-len', help='max length of 1 episode', default=501)
